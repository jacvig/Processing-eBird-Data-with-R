...
## e.g. eBird data for Andalucia, Spain from Jan 2016 - Sept 2022. 
...
# Import data 
# eBird files can come in .csv or .txt.
...
data <- data |>
  select(GLOBAL.UNIQUE.IDENTIFIER, COMMON.NAME, SCIENTIFIC.NAME, 
         OBSERVATION.COUNT, COUNTY, LOCALITY, OBSERVATION.DATE,
         TIME.OBSERVATIONS.STARTED, SAMPLING.EVENT.IDENTIFIER, PROTOCOL.TYPE,
         DURATION.MINUTES, EFFORT.DISTANCE.KM, NUMBER.OBSERVERS, 
         ALL.SPECIES.REPORTED, GROUP.IDENTIFIER, TRIP.COMMENTS, SPECIES.COMMENTS)


# Filter by date. First separate Year, Month, and Day into their own columns
...
# If you get the error "NAs introduced by coercion", you can ignore.
...
# Rename columns
data<-data |>
  rename(Duration = DURATION.MINUTES, Distance = EFFORT.DISTANCE.KM, 
...
# Look at total number of species seen in each week
# need to create weeks

Week <-Ronda |>
  filter(Day %in% 1:7) # I will only look at the first week

Tot <-Week |>
  group_by(Year, Day) |>
  summarise(count = n_distinct(COMMON.NAME)) |>
  ungroup()
  
ggplot(Tot, aes(x= Day, y = count))+
  geom_bar(stat = "identity")+
  facet_wrap(~Year)+
  labs(title = "Unique species observations across September",
       x= "Day of month",
       y= "Number of species")+
  theme_bw()
